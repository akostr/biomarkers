...
#include "ui_tree_widget_merged_tables.h"
...
TreeWidgetMergedTables::TreeWidgetMergedTables(QWidget* parent)
	: QWidget(parent)
	, ui(new Ui::TreeWidgetMergedTables)
{
	SetupUi();
	ConnectSignals();
}

TreeWidgetMergedTables::~TreeWidgetMergedTables()
{
	delete ui;
}

void TreeWidgetMergedTables::SetupUi()
{
	ui->setupUi(this);
	setStyleSheet(Style::Genesis::GetUiStyle());
	mModel = new TreeModelDynamicNumericTables(Names::Group::MergedTableIdentified, this);
...
void TreeWidgetMergedTables::ConnectSignals()
{
	...
	connect(ui->rejectActionBtn, &QPushButton::clicked, this, &TreeWidgetMergedTables::ExitAction);
	...
	connect(mModel, &ModelClass::rowsInserted, this, &TreeWidgetMergedTables::UpdateLabelAndCheckbox);
	connect(mModel, &ModelClass::rowsRemoved, this, &TreeWidgetMergedTables::UpdateLabelAndCheckbox);
	connect(mModel, &ModelClass::modelReset, this, &TreeWidgetMergedTables::UpdateLabelAndCheckbox);
	connect(mModel, &ModelClass::dataChanged, this, &TreeWidgetMergedTables::OnDataChanged);
	...
	{
		connect(menu->addAction(tr("Edit group")), &QAction::triggered, this, &TreeWidgetMergedTables::PickedEditGroupAction);
		connect(menu->addAction(tr("Export")), &QAction::triggered, this, &TreeWidgetMergedTables::PickedExportGroupAction);
		connect(menu->addAction(tr("Remove")), &QAction::triggered, this, &TreeWidgetMergedTables::PickedRemoveGroupAction);
	}

}

void TreeWidgetMergedTables::ActionAssignAndEnter(QString buttonText, QString forWhatAction, std::function<void()> func, std::function<void()> prepare, std::function<void()> done)
{
	prepare();
	mDoneAction = done;
...
void TreeWidgetMergedTables::ExitAction()
{
	if (mActionConnection)
		disconnect(mActionConnection);
	ui->rejectActionBtn->hide();
...
void TreeWidgetMergedTables::PickedEditGroupAction()
{
	ActionAssignAndEnter(tr("Edit group"), tr("for groups"),
		[&]()
		{
			auto projectId = GenesisContextRoot::Get()->ProjectId();
            API::REST::GetTableGroups(projectId, Names::Group::MergedTableIdentified, [this](QNetworkReply* r, QJsonDocument doc)
				{
...
void TreeWidgetMergedTables::ShowEditGroupDialog(const QList<int>& ids, const std::map<QString, int>& groupInfo)
{
	if (ids.isEmpty())
	{
...
void TreeWidgetMergedTables::PickedExportGroupAction()
{
...
void TreeWidgetMergedTables::PickedRemoveGroupAction()
{
	ActionAssignAndEnter(tr("Remove"), tr("for removing"), [this]()
		{
			...
			Dialogs::Templates::TreeConfirmation::DeleteTablesWithDependencies(infos, this, "TreeModelDynamicMergedTables");
			ExitAction();
		});
}

void TreeWidgetMergedTables::UpdateCountLabel()
{
...
void TreeWidgetMergedTables::ItemEdit(TreeModelItem* item)
{
...
void TreeWidgetMergedTables::ItemExport(TreeModelItem* item)
{
...
void TreeWidgetMergedTables::ItemRemove(TreeModelItem* item)
{
	Dialogs::Templates::TreeConfirmation::DeleteTablesWithDependencies({ {item->GetData("_id").toInt(), item->GetData("_title").toString()} }, this, "TreeModelDynamicMergedTables");
}

void TreeWidgetMergedTables::EditGroup(TreeModelItem* item)
{
	auto projectId = GenesisContextRoot::Get()->ProjectId();
    API::REST::GetTableGroups(projectId, Names::Group::MergedTableIdentified, [this, item](QNetworkReply* r, QJsonDocument doc)
		{
			std::map<QString, int> idByNames;
			auto jdata = doc.object()["data"].toArray();
			for (int i = 0; i < jdata.size(); i++)
...
void TreeWidgetMergedTables::EditTable(TreeModelItem* item)
{
...
void TreeWidgetMergedTables::EditGroupTitle(TreeModelItem* item)
{
...
void TreeWidgetMergedTables::UpdateMainCheckBox(QString forWhat)
{
...
void TreeWidgetMergedTables::OnDataChanged(const QModelIndex& topLeft, const QModelIndex& bottomRight, const QList<int>& roles)
{
...
void TreeWidgetMergedTables::UpdateLabelAndCheckbox()
{
	UpdateCountLabel();
	UpdateMainCheckBox(ui->pickAllCheckBox->property("for_what").toString());
}
