...
TreeWidgetImportedTables::TreeWidgetImportedTables(QWidget* parent)
	: QWidget(parent)
...
TreeWidgetImportedTables::~TreeWidgetImportedTables()
{
	delete ui;
}

void TreeWidgetImportedTables::SetupUi()
{
	ui->setupUi(this);
	setStyleSheet(Style::Genesis::GetUiStyle());
	mModel = new TreeModelDynamicNumericTables(Names::Group::ImportedTable, this);
	...
	connect(menu->addAction(tr("Edit group")), &QAction::triggered, this, &TreeWidgetImportedTables::PickedEditGroupAction);
	connect(menu->addAction(tr("Export")), &QAction::triggered, this, &TreeWidgetImportedTables::PickedExportGroupAction);
	connect(menu->addAction(tr("Remove")), &QAction::triggered, this, &TreeWidgetImportedTables::PickedRemoveGroupAction);
	...
	connect(ui->rejectActionBtn, &QPushButton::clicked, this, &TreeWidgetImportedTables::ExitAction);
	...
	connect(mModel, &ModelClass::rowsInserted, this, &TreeWidgetImportedTables::UpdateLabelAndCheckbox);
	connect(mModel, &ModelClass::rowsRemoved, this, &TreeWidgetImportedTables::UpdateLabelAndCheckbox);
	connect(mModel, &ModelClass::modelReset, this, &TreeWidgetImportedTables::UpdateLabelAndCheckbox);
	connect(mModel, &ModelClass::dataChanged, this, &TreeWidgetImportedTables::OnDataChanged);
...
void TreeWidgetImportedTables::ActionAssignAndEnter(QString buttonText, QString forWhatAction, std::function<void()> func, std::function<void()> prepare, std::function<void()> done)
{
	prepare();
	mDoneAction = done;
...
void TreeWidgetImportedTables::ExitAction()
{
	if (mActionConnection)
		disconnect(mActionConnection);
	ui->rejectActionBtn->hide();
...
void TreeWidgetImportedTables::PickedEditGroupAction()
{
...
void TreeWidgetImportedTables::ShowEditGroupDialog(const QList<int>& ids, const std::map<QString, int>& groupInfo)
{
	if (ids.isEmpty())
	{
...
void TreeWidgetImportedTables::PickedExportGroupAction()
{
...
void TreeWidgetImportedTables::PickedRemoveGroupAction()
{
...
void TreeWidgetImportedTables::UpdateCountLabel()
{
...
void TreeWidgetImportedTables::ItemEdit(TreeModelItem* item)
{
...
void TreeWidgetImportedTables::ItemExport(TreeModelItem* item)
{
...
void TreeWidgetImportedTables::ItemRemove(TreeModelItem* item)
{
...
void TreeWidgetImportedTables::EditGroup(TreeModelItem* item)
{
    auto projectId = GenesisContextRoot::Get()->ProjectId();
    QString tableType;
    auto module = GenesisContextRoot::Get()->GetContextModules()->GetData(Names::ModulesContextTags::kModule).value<Names::ModulesContextTags::Module>();
    if(module == Names::ModulesContextTags::MBiomarkers)
      tableType = Names::Group::ImportedTableIdentified;
    else
      tableType = Names::Group::ImportedTable;
    API::REST::GetTableGroups(projectId, tableType, [this, item](QNetworkReply* r, QJsonDocument doc)
		{
			std::map<QString, int> idByNames;
			auto jdata = doc.object()["data"].toArray();
			for (int i = 0; i < jdata.size(); i++)
...
void TreeWidgetImportedTables::EditTable(TreeModelItem* item)
{
...
void TreeWidgetImportedTables::EditGroupTitle(TreeModelItem* item)
{
...
void TreeWidgetImportedTables::UpdateMainCheckBox(QString forWhat)
{
...
void TreeWidgetImportedTables::OnDataChanged(const QModelIndex& topLeft, const QModelIndex& bottomRight, const QList<int>& roles)
{
...
void TreeWidgetImportedTables::UpdateLabelAndCheckbox()
{
	UpdateCountLabel();
	UpdateMainCheckBox(ui->pickAllCheckBox->property("for_what").toString());
}
