...
#include "ui_tree_widget_identified_tables.h"
...
namespace
{
  const QString kValuesLibraryGroupId = "_values_library_group_ids";
  const QString kCheckable = "checkable";
}

TreeWidgetIdentifiedTables::TreeWidgetIdentifiedTables(QWidget* parent)
  : QWidget(parent)
  , ui(new Ui::TreeWidgetIdentifiedTables)
{
  SetupUi();
  ConnectSignals();
}

TreeWidgetIdentifiedTables::~TreeWidgetIdentifiedTables()
{
  delete ui;
}

void TreeWidgetIdentifiedTables::SetupUi()
{
  ui->setupUi(this);
  setStyleSheet(Style::Genesis::GetUiStyle());
  mModel = new TreeModelDynamicNumericTables(Names::Group::DataTableIdentified, this);
...
void TreeWidgetIdentifiedTables::ConnectSignals()
{
  ...
  connect(ui->rejectActionBtn, &QPushButton::clicked, this, &TreeWidgetIdentifiedTables::ExitAction);
  ...
  connect(mModel, &ModelClass::rowsInserted, this, &TreeWidgetIdentifiedTables::UpdateLabelAndCheckbox);
  connect(mModel, &ModelClass::rowsRemoved, this, &TreeWidgetIdentifiedTables::UpdateLabelAndCheckbox);
  connect(mModel, &ModelClass::modelReset, this, &TreeWidgetIdentifiedTables::UpdateLabelAndCheckbox);
  connect(mModel, &ModelClass::dataChanged, this, &TreeWidgetIdentifiedTables::OnDataChanged);
  ...
  {
    connect(menu->addAction(tr("Merge tables")), &QAction::triggered, this, &TreeWidgetIdentifiedTables::PickedMergeTablesAction);
    connect(menu->addAction(tr("Edit group")), &QAction::triggered, this, &TreeWidgetIdentifiedTables::PickedEditGroupAction);
    connect(menu->addAction(tr("Export")), &QAction::triggered, this, &TreeWidgetIdentifiedTables::PickedExportGroupAction);
    connect(menu->addAction(tr("Remove")), &QAction::triggered, this, &TreeWidgetIdentifiedTables::PickedRemoveGroupAction);
  }

}

void TreeWidgetIdentifiedTables::ActionAssignAndEnter(QString buttonText, QString forWhatAction, std::function<void()> func, std::function<void()> prepare, std::function<void()> done)
{
  prepare();
  mDoneAction = done;
...
void TreeWidgetIdentifiedTables::ExitAction()
{
  if (mActionConnection)
    disconnect(mActionConnection);
  ui->rejectActionBtn->hide();
...
void TreeWidgetIdentifiedTables::PickedEditGroupAction()
{
  ActionAssignAndEnter(tr("Edit group"), tr("for groups"),
    [&]()
    {
      auto projectId = GenesisContextRoot::Get()->ProjectId();
      API::REST::GetTableGroups(projectId, Names::Group::DataTableIdentified, [this](QNetworkReply* r, QJsonDocument doc)
        {
...
void TreeWidgetIdentifiedTables::PickedMergeTablesAction()
{
  ActionAssignAndEnter(tr("Merge"), tr("for tables"),
    [&]()
    {
      ...
      connect(dialog, &WebDialogJoinTables::Accepted, this, &TreeWidgetIdentifiedTables::ExitAction);
      dialog->Open();
    });
  ui->pickAllCheckBox->hide();
}

void TreeWidgetIdentifiedTables::ShowEditGroupDialog(const QList<int>& ids, const std::map<QString, int>& groupInfo)
{
  if (ids.isEmpty())
  {
...
void TreeWidgetIdentifiedTables::PickedExportGroupAction()
{
...
void TreeWidgetIdentifiedTables::PickedRemoveGroupAction()
{
  ActionAssignAndEnter(tr("Remove"), tr("for removing"), [this]()
    {
      ...
      Dialogs::Templates::TreeConfirmation::DeleteTablesWithDependencies(infos, this, "TreeModelDynamicIdentifiedTables");
      ExitAction();
    });
}

void TreeWidgetIdentifiedTables::UpdateCountLabel()
{
...
void TreeWidgetIdentifiedTables::ItemEdit(TreeModelItem* item)
{
...
void TreeWidgetIdentifiedTables::ItemExport(TreeModelItem* item)
{
...
void TreeWidgetIdentifiedTables::ItemRemove(TreeModelItem* item)
{
  Dialogs::Templates::TreeConfirmation::DeleteTablesWithDependencies({ {item->GetData("_id").toInt(), item->GetData("_title").toString()} }, this, "TreeModelDynamicIdentifiedTables");
}

void TreeWidgetIdentifiedTables::EditGroup(TreeModelItem* item)
{
  auto projectId = GenesisContextRoot::Get()->ProjectId();
  API::REST::GetTableGroups(projectId, Names::Group::DataTableIdentified, [this, item](QNetworkReply* r, QJsonDocument doc)
    {
      std::map<QString, int> idByNames;
      auto jdata = doc.object()["data"].toArray();
      for (int i = 0; i < jdata.size(); i++)
...
void TreeWidgetIdentifiedTables::EditTable(TreeModelItem* item)
{
...
void TreeWidgetIdentifiedTables::EditGroupTitle(TreeModelItem* item)
{
...
void TreeWidgetIdentifiedTables::UpdateMainCheckBox(QString forWhat)
{
...
void TreeWidgetIdentifiedTables::OnDataChanged(const QModelIndex& topLeft, const QModelIndex& bottomRight, const QList<int>& roles)
{
  if (roles.contains(Qt::CheckStateRole))
  {
    UpdateMainCheckBox(ui->pickAllCheckBox->property("for_what").toString());
    DisableCheckboxesByLibraryGroupIds();
    ui->treeView->update();
  }
}

void TreeWidgetIdentifiedTables::UpdateLabelAndCheckbox()
{
  UpdateCountLabel();
  UpdateMainCheckBox(ui->pickAllCheckBox->property("for_what").toString());
}

void TreeWidgetIdentifiedTables::DisableCheckboxesByLibraryGroupIds()
{
  QSet<int> filterIds;
  for (const auto item : mModel->GetCheckedItems())
  {
    const auto groupIds = item->GetData(kValuesLibraryGroupId).toList();
    std::transform(groupIds.begin(), groupIds.end(),
      std::inserter(filterIds, filterIds.end()),
      [](const QVariant& id) { return id.toInt(); });
  }

  for (const auto item : mModel->GetRoot()->GetChildrenRecursive())
  {
    if (item->GetCheckState() == Qt::Checked)
      continue;
    const auto treeItemData = item->GetData(kValuesLibraryGroupId).toList();
    QList<int> ids;
    std::transform(treeItemData.begin(), treeItemData.end(),
      std::back_inserter(ids), [](const QVariant& id) { return id.toInt(); });
    const auto findIt = std::find_if(treeItemData.begin(), treeItemData.end(),
      [&](const QVariant& id) { return filterIds.contains(id.toInt()); });
    item->SetData(kCheckable, findIt == treeItemData.end());
  }
}
